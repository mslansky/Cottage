...
export default class ItemPage extends React.Component{
  ...
  addToCartButton(items){
    if(!!this.context.user.id){
      return (
        <button className="add"  key={items.id} onClick={() => this.addToCart(items.id)}>Add to Cart</button>
        )
    }else{
      return (
        <Link to="/Register" key={items.id}>
          <button className="add">Login & add</button>
        </Link>
      )
    }
  }
    ...
    return(
      <div className="items">
        ...
        {this.state.items.reverse().map((items, index) => (
          <div className="item-pic" key={items.id}>
          ...
          {this.state.items.reverse().map((items, index) => (
          <div className="item-price" key={items.id}>
            <p>{items.price}</p>
          </div>
             ))}
          ...
          {this.state.items.reverse().map((items, index) => (
            <Link to={`/Store/${items.shopId}`} key={items.id}><h1>{items.shop}</h1></Link>
            ))}
          </div>
