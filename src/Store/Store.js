  ...
  render(){
    return(
      <div className="store-page">
        ...
        <div className="store-img" key={shops.id}>
          <img src = {shops.picUrl} className="storePic" alt={shops.name}/>
        </div>
         ))}

        {this.state.shops.reverse().map((shops, index) => (
        <div className="store-title" key={shops.id}>
          <h1> {shops.name}</h1>
        </div>
         ))}
      ...
      {this.state.shops.reverse().map((shops, index) => (
        <div className="store-desc" key={shops.id}>
          <p> {shops.desc}</p>
        </div>
         ))}
        ...
        {this.state.items.reverse().map((item, index) => (<ItemSquare item={item} key={item.id}/>))}
        </div>
